...
const StaffKYC = () => {
  ...
  // Fetch pending KYC requests (staff can only see pending ones)
  ...
  // Recommend KYC mutation (staff recommends for admin review)
  const recommendKycMutation = useMutation({
    mutationFn: async ({ userId, recommendation }: { userId: string; recommendation: 'recommended_approve' | 'recommended_reject' }) => {
      const { data, error } = await supabase.rpc('approve_kyc', {
        p_user_id: userId,
        p_new_status: recommendation,
        p_reason: null
      });
      if (error) throw error;
      return data;
    ...
    onSuccess: (_, variables) => {
      ...
      const action = variables.recommendation === 'recommended_approve' 
        ? 'recommended for approval' 
        : 'recommended for rejection';
      toast.success(`KYC ${action} — sent to admin`);
  ...
  const handleRecommend = (type: 'recommended_approve' | 'recommended_reject') => {
    if (!selectedRequest) return;
    recommendKycMutation.mutate({ userId: selectedRequest.id, recommendation: type });
  };
      ...
      <div className="h-[calc(100vh-8rem)] flex flex-col">
        ...
        <div className="p-3 rounded-lg bg-cyan-500/5 border border-cyan-500/20 mb-4">
          <p className="text-xs text-cyan-400 font-mono">
            <strong>VIEW & RECOMMEND ONLY:</strong> Review documents and recommend for admin approval or rejection. You cannot approve or reject directly.
          </p>
        </div>
              ...
              <>
                ...
                <div className="flex gap-3">
                  <Button
                    ...
                    onClick={() => handleRecommend('recommended_reject')}
                    disabled={recommendKycMutation.isPending}
                  >
                    {recommendKycMutation.isPending ? (
                  ...
                  <Button
                    ...
                    onClick={() => handleRecommend('recommended_approve')}
                    disabled={recommendKycMutation.isPending}
                  >
                    {recommendKycMutation.isPending ? (
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ...
                <p className="text-xs text-slate-500 text-center">
                  Your recommendation will be sent to admin for final decision.
                </p>
              </>
            ) : (
